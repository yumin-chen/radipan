...
Radipan comes with [HyperScript](https://github.com/hyperhype/hyperscript)-compatible API, which you can import and use like `h(tag, attrs, [text?, Elements?,...])`:
...
You can use recipes directly within the `css` prop. Recipes are a way to define styles for different variants of a component. They offer better performance, developer experience, and composability.
...
- The transpiler uses Babel to handle the `css`-to-`className` transformation. It then runs your code at build time, and turns any dynamic references into static values. If you need real dynamic styles, you might need to move any runtime references (e.g., reference of values from hooks) into `styles` prop.
